...
<div class="main-section">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="fw-bold mb-0">Edit Office</h4>
    ...
    <div class="card shadow-sm rounded border mb-4">
        <div class="card-body p-4">
            <form action="{{ route('admin.offices.update', $office) }}" method="POST" class="row g-3">
                @csrf
                @method('PUT')
                ...
                <div class="col-md-6">
                    ...
                    <input type="time" class="form-control @error('opening_time') is-invalid @enderror"
                        id="opening_time" name="opening_time" value="{{ old('opening_time', $office->opening_time ? date('H:i', strtotime($office->opening_time)) : '') }}">
                    @error('opening_time')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="time" class="form-control @error('closing_time') is-invalid @enderror"
                        id="closing_time" name="closing_time" value="{{ old('closing_time', $office->closing_time ? date('H:i', strtotime($office->closing_time)) : '') }}">
                    @error('closing_time')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12 d-flex gap-2 justify-content-end mt-2">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save me-1"></i> Update Office
                    </button>
                </div>
            </form>
        </div>
