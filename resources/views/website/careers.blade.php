            ...
            <div class="cards-grid">
                @foreach($lifeAtVyapto as $item)
                    <div class="card-item">
                        ...
                        <h3>{{ $item->title }}</h3>
                        <p>{{ $item->excerpt }}</p>
                        @if($item->content)<div class="blog-content">{!! $item->content !!}</div>@endif
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="cards-grid">
                @foreach($deliveryPartners as $item)
                    <div class="card-item">
                        ...
                        @if($item->content)<div class="blog-content">{!! $item->content !!}</div>@endif
                        @if($item->link)
                            <a href="{{ $item->link }}" class="cta-btn" style="margin-top:12px;display:inline-flex;">Apply Now</a>
                        @else
                            <a href="{{ route('portal.register') }}" class="cta-btn" style="margin-top:12px;display:inline-flex;">Register Now</a>
                        @endif
                    </div>
                @endforeach
            </div>
            ...
            <div class="cards-grid">
                @foreach($openings as $job)
                    <div class="card-item">
                        ...
                        @endif
                        <p>{{ $job->excerpt }}</p>
                        @if($job->content)<div class="blog-content">{!! $job->content !!}</div>@endif
