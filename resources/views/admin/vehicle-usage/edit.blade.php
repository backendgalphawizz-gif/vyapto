...
<div class="main-section">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="fw-bold mb-0">Edit Vehicle Usage #{{ $vehicleUsage->id }}</h4>
    ...
    <div class="card shadow-sm rounded border mb-4">
        <form action="{{ route('admin.vehicle-usage.update', $vehicleUsage) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="card-body">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="vehicle_number" class="form-control @error('vehicle_number') is-invalid @enderror" value="{{ old('vehicle_number', $vehicleUsage->vehicle_number) }}" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="user_id" class="form-select @error('user_id') is-invalid @enderror" required>
                            ...
                            <option value="{{ $user->id }}" {{ old('user_id', $vehicleUsage->user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }} - {{ $user->email }}</option>
                            @endforeach
                        </select>
                        @error('user_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" name="kms" class="form-control @error('kms') is-invalid @enderror" value="{{ old('kms', $vehicleUsage->kms) }}" step="0.01" min="0">
                        @error('kms')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
